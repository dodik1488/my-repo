...
class FiltersMod(loader.Module):
    ...
    async def filtercmd(self, message):
        ...
        filters = self.db.get("FilterModule", "filters", {})
        if not args:
        ...
        if not message.is_reply:
            if len(args) == 1:
                ...
                value = args[1]
                msg_to_log = await self.db.store_asset(value)
        else:
            value = await message.get_reply_message()
            msg_to_log = await self.db.store_asset(value)
        filters[chatid][name] = msg_to_log
        self.db.set("FilterModule", "filters", filters)
    ...
    async def filterscmd(self, message):
        """Shows saved filters."""
        filters = ""
        filt = self.db.get("FilterModule", "filters", {})
        chatid = str(message.chat_id)
        try:
    ...
    async def watchout(self, message):
        ...
        for key in filters[chatid]:
            if key in args:
                ...
                value = await self.db.fetch_asset(id)
